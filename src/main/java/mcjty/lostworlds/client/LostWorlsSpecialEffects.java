...
public class LostWorlsSpecialEffects extends DimensionSpecialEffects {
    ...
    public boolean renderSky(ClientLevel level, int ticks, float partialTick, PoseStack poseStack, Camera camera, Matrix4f projectionMatrix, boolean isFoggy, Runnable setupFog) {
        renderSkyInt(level, ticks, partialTick, poseStack, camera, projectionMatrix, isFoggy, setupFog);
        return true;
    }
    ...
    private void renderSkyInt(ClientLevel level, int ticks, float partialTick, PoseStack poseStack, Camera camera, Matrix4f projectionMatrix, boolean isFoggy, Runnable setupFog) {
        ...
        if (!isFoggy) {
            ...
            if (fogtype != FogType.POWDER_SNOW && fogtype != FogType.LAVA && !this.doesMobEffectBlockSky(camera)) {
                if (minecraft.level.effects().skyType() == DimensionSpecialEffects.SkyType.NORMAL) {
                    ...
                    double d0 = minecraft.player.getEyePosition(partialTick).y - (-63);//level.getLevelData().getHorizonHeight(level);
                    if (d0 < 0.0D) {
                        poseStack.pushPose();
