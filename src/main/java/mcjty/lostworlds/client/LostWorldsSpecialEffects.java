...
public class LostWorldsSpecialEffects extends DimensionSpecialEffects {
    ...
    public Vec3 getBrightnessDependentFogColor(Vec3 color, float brightness) {
        if (fogColor == FogColor.NONE) {
            if (type == LostWorldType.ISLANDS || type == LostWorldType.ISLANDS_WATER || type == LostWorldType.SPHERES || type == LostWorldType.NORMAL) {
    ...
    public boolean hasGround() {
        if (type == LostWorldType.ISLANDS || type == LostWorldType.ISLANDS_WATER || type == LostWorldType.SPHERES) {
            return false;
        }
